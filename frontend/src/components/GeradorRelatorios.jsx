...
export default function GeradorRelatorios() {
  ...
  const pedirParaIAGerar = async (tipoRelatorio) => {
    ...
    try {
      ...
      const resposta = await fetch(`http://localhost:8000/api/relatorios/gerar?tipo=${tipoRelatorio}`, {
        headers: {
  ...
  const enviarMensagemChat = async (e) => {
    ...
    try {
      ...
      const resposta = await fetch('http://localhost:8000/api/relatorios/chat', {
        method: 'POST',
        headers: { 
